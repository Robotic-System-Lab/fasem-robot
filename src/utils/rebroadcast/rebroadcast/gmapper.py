...
class GMappingNode(Node):
	def __init__(self):
		...
		source_odom = self.get_parameter('source_odom').value
		source_scan = self.get_parameter('source_scan').value
		...
		self.create_subscription(Odometry, source_odom, self.callback_odom, 10)
		self.create_subscription(LaserScan, source_scan, self.callback_scan, 10)
		self.create_subscription(Float64, '/segmentation_result', self.callback_start, 10)
