...
bool FrontierHelper::checkRectangularFootprintCollision(
  ...
  const nav_msgs::msg::OccupancyGrid & map_data)
{
  ...
  double safe_length = robot_length + 2 * safety_margin;
  double safe_width = robot_width + 2 * safety_margin;
  
  // Calculate footprint corners (assuming robot is axis-aligned)
  double half_length = safe_length / 2.0;
  double half_width = safe_width / 2.0;
  
  std::vector<std::pair<double, double>> corners = {
    {position_x - half_length, position_y - half_width},  // Bottom-left
    {position_x + half_length, position_y - half_width},  // Bottom-right
    {position_x + half_length, position_y + half_width},  // Top-right
    {position_x - half_length, position_y + half_width}   // Top-left
  };
  
  // Check if any corner is outside map bounds
  for (const auto & corner : corners) {
    if (isPositionOutsideMap(map_data, corner.first, corner.second)) {
      return true;  // Collision with map boundary
    }
  }
  
  // Convert world coordinates to grid coordinates and check for obstacles
  ...
  // Sample points within the footprint area
  double sample_resolution = resolution / 2.0;  // Higher resolution sampling
  
  for (double x = position_x - half_length; x <= position_x + half_length; x += sample_resolution) {
    for (double y = position_y - half_width; y <= position_y + half_width; y += sample_resolution) {
      ...
      if (grid_x >= 0 && grid_x < width && grid_y >= 0 && grid_y < height) {
        ...
        // Check for obstacle (value >= 65 is usually considered obstacle)
...
bool FrontierHelper::isFootprintSafe(
  const Cell & frontier, const Coord & robot_position,
  double robot_length, double robot_width,
  double safety_margin,
  const nav_msgs::msg::OccupancyGrid & map_data)
{
  // Convert frontier cell to world coordinates
  auto [frontier_x, frontier_y] = cellToWorld(frontier, map_data);
  
  // Check collision at frontier position
  if (checkRectangularFootprintCollision(
        frontier_x, frontier_y, robot_length, robot_width, 
        safety_margin, map_data)) {
    return false;
  }
  
  // Additional check: ensure there's enough maneuvering space
  // Check positions around the frontier for rotation clearance
  double maneuver_check_radius = std::max(robot_length, robot_width) / 2.0 + safety_margin;
  
  // Calculate distance from robot to frontier
  double distance_to_frontier = std::hypot(
    frontier_x - robot_position.first, frontier_y - robot_position.second);
  
  // For close frontiers, be more strict about maneuvering space
  double strictness_factor = (distance_to_frontier < 2.0) ? 1.5 : 1.0;
  double adjusted_radius = maneuver_check_radius * strictness_factor;
  
  std::vector<std::pair<double, double>> check_positions = {
    {frontier_x + adjusted_radius, frontier_y},                    // Right
    {frontier_x - adjusted_radius, frontier_y},                    // Left
    {frontier_x, frontier_y + adjusted_radius},                    // Up
    {frontier_x, frontier_y - adjusted_radius},                    // Down
    {frontier_x + adjusted_radius * 0.707, frontier_y + adjusted_radius * 0.707},  // Diagonal NE
    {frontier_x - adjusted_radius * 0.707, frontier_y - adjusted_radius * 0.707}   // Diagonal SW
  };
  
  for (const auto & pos : check_positions) {
    if (checkRectangularFootprintCollision(
          pos.first, pos.second, robot_length, robot_width,
          safety_margin * 0.5, map_data)) {
      return false;  // Not enough maneuvering space
    }
  }
  
  return true;  // Frontier is safe and has maneuvering space
}
