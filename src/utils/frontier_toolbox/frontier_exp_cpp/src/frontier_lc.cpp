...
public:
  ...
  : nav2_util::LifecycleNode("frontier_explorer"), tf_buffer_(get_clock()), tf_listener_(tf_buffer_)
  {
    ...
    robot_radius_des.description =
      "The radius around the robot in [m] to determine if a frontier is too close (DEPRECATED - use robot dimensions instead)";
    declare_parameter("robot_radius", 0.35, robot_radius_des);
    
    // Add new robot footprint parameters
    auto robot_length_des = rcl_interfaces::msg::ParameterDescriptor{};
    auto robot_width_des = rcl_interfaces::msg::ParameterDescriptor{};
    auto footprint_safety_margin_des = rcl_interfaces::msg::ParameterDescriptor{};
    
    robot_length_des.description = "The length (x-dimension) of the robot in meters";
    declare_parameter("robot_length", 0.99, robot_length_des);
    robot_width_des.description = "The width (y-dimension) of the robot in meters";
    declare_parameter("robot_width", 0.67, robot_width_des);
    footprint_safety_margin_des.description = "Additional safety margin around robot footprint in meters";
    declare_parameter("footprint_safety_margin", 0.15, footprint_safety_margin_des);
...
private:
  ...
  // New footprint-based parameters
  double robot_length_;
  double robot_width_;
  double footprint_safety_margin_;
  ...
  {
    std::string base_frame = is_sim_ ? "fasem_footprint" : "fasem_link";
    try {
  ...
  std::vector<Cell> cleanupFrontiers(const std::vector<Cell> & frontiers)
  {
    ...
    int total_frontiers = frontiers.size();
    ...
    // Log filtering results with footprint information
    RCLCPP_INFO(get_logger(), 
      "Frontier filtering: %d/%d frontiers removed due to rectangular footprint collision check "
      "(robot: %.2fm x %.2fm + %.2fm safety margin)", 
      filtered_count, total_frontiers, robot_length_, robot_width_, footprint_safety_margin_);
    
    // Convert the vector to a string to print out if needed (DEBUG level)
    std::stringstream ss;
    ss << "Safe frontier cells: ";
  ...
  {
    // Use new footprint-aware collision checking instead of simple radius
    return !FrontierHelper::isFootprintSafe(
      frontier, robot_position_, 
      robot_length_, robot_width_,
      footprint_safety_margin_, map_data_);
  }
