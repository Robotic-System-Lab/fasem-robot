...
def pure_pursuit(current_x, current_y, current_heading, path, index, lookahead_distance, speed, angular_limiter):
    closest_point = None
    v = speed
    ...
    # Batasi kecepatan angular maksimum untuk mencegah robot jatuh
    max_angular_velocity = angular_limiter  # Maksimum angular velocity (7.5°/detik)
    
    if desired_steering_angle > angular_limiter or desired_steering_angle < -angular_limiter:
        sign = 1 if desired_steering_angle > 0 else -1
        desired_steering_angle = sign * max_angular_velocity
        v = v * 0.3  # Kurangi kecepatan linear lebih banyak saat berbelok tajam
    
    # Tambahan pembatasan untuk memastikan angular velocity tidak terlalu tinggi
    if abs(desired_steering_angle) > max_angular_velocity:
        desired_steering_angle = max_angular_velocity * (1 if desired_steering_angle > 0 else -1)
...
def costmap(data,width,height,resolution,expansion_size):
    ...
    # Ekspansi obstacle dengan area yang lebih luas
    ...
    # Tambahan: ekspansi kedua dengan radius lebih kecil untuk gradual safety zone
    wall_expanded = np.where(data == 100)
    for i in range(-3, 4):  # Ekspansi tambahan 3 pixel
        for j in range(-3, 4):
            if i == 0 and j == 0:
                continue
            x = wall_expanded[0]+i
            ...
            x = np.clip(x, 0, height-1)
            y = np.clip(y, 0, width-1)
            # Buat zone peringatan dengan nilai 50 (antara free space dan obstacle)
            if data[x,y] == 0:
                data[x,y] = 50
...
def exploration(data,width,height,resolution,column,row,originX,originY,expansion_size,target_error):
        ...
        data[data > 20] = 1 # Threshold lebih rendah: 0 = gidilebilir, >20 = gidilemez (termasuk zone peringatan)
...
def localControl(scan, robot_r, angular_limiter):
    v = None
    w = None
    # Memperluas area deteksi obstacle dari depan kanan (0-90°)
    for i in range(60):  # Diperluas dari 60 ke 90 derajat
        if scan[i] < robot_r * 1.5:  # Jarak deteksi diperbesar 1.5x
            v = 0.08
            w = -angular_limiter
            break
    if v == None:
        # Memperluas area deteksi obstacle dari depan kiri (270-360°)
        for i in range(300, 360):  # Diperluas dari 300 ke 270 derajat
            if scan[i] < robot_r * 1.5:  # Jarak deteksi diperbesar 1.5x
                v = 0.08
                w = angular_limiter
                break
...
class navigationControl(Node):
    def __init__(self):
        ...
        # Declare parameters with default values from the YAML file
        self.declare_parameter('lookahead_distance', 0.24)
        self.declare_parameter('speed', 0.2)
        self.declare_parameter('angular_limiter', 8)
        self.declare_parameter('expansion_size', 15)
        self.declare_parameter('target_error', 0.15)
        self.declare_parameter('robot_r', 1)
        
        # Get parameter values
        self.lookahead_distance = self.get_parameter('lookahead_distance').value
        self.speed = self.get_parameter('speed').value
        self.angular_limiter = math.pi / self.get_parameter('angular_limiter').value
    ...
    def exp(self):
        ...
        while True: #Sensor verileri gelene kadar bekle.
            ...
            #Rota Takip Blok Baslangic
                ...
                v, w = localControl(self.scan, self.robot_r, self.angular_limiter)
                if v == None:
                    v, w,self.i = pure_pursuit(self.x,self.y,self.yaw,self.path,self.i,self.lookahead_distance,self.speed,self.angular_limiter)
                
                # Tambahan pembatasan untuk memastikan stabilitas robot
                max_angular_velocity = self.angular_limiter  # Batas maksimum angular velocity (7.5°/detik)
                if w is not None and abs(w) > max_angular_velocity:
                    w = max_angular_velocity * (1 if w > 0 else -1)
